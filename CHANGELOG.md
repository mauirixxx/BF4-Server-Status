...
### Added
- Added fair PostgreSQL-backed Keeper request scheduling so healthy distributed workers with pending work cannot starve under sustained cluster load.
- Added adaptive distributed presence health based on observed Keeper lane cadence instead of relying on a fixed 15-minute snapshot-freshness assumption.
- Added durable last-known-good presence aggregate storage so a newly elected Discord leader can immediately recover the previous leader's valid player aggregate.
- Added persistent per-worker Keeper lane telemetry for assigned servers, successes, failures, skips, elapsed time, rate-gate wait time, observed cadence, and sweep completion time.
- Added adaptive persistent player-list ETA calculation based on observed process-local player-display start-to-start cadence.
- Added Alembic revisions `0018_v3_0_1_keeper_fair` and `0019_v3_0_1_presence_health`.

### Changed
- Keeper bulk and fast lanes now use durable fair waiter queues to decide which worker receives the next request grant, while the existing PostgreSQL global and lane timestamps continue to enforce the hard traffic ceilings.
- Distributed presence classifies snapshots using adaptive bulk/fast freshness horizons derived from recent meaningful Keeper cadence telemetry.
- Presence health continues to require 90% usable coverage, but “usable” is now evaluated against the adaptive lane horizons rather than a single fixed snapshot age.
- Persisted presence fallback validity is bounded and derived from the observed Keeper cadence, allowing safe continuity through leader handoffs without retaining stale aggregates indefinitely.
- Zero-work Keeper sweeps during drains, restarts, and rolling deployments no longer overwrite the last meaningful non-zero cadence observation.
- Player-list ETA uses up to six observed cadence samples, retains a fallback before observations exist, and clamps the resulting estimate to a safe 60–900 second range.
- Final `v3.0.1` promotes the validated `v3.0.1-pr2c` runtime unchanged except for release-version and Docker image metadata.

### Reliability
- Eliminated first-arriver-wins PostgreSQL Keeper gate behavior that allowed some healthy HRW owners to repeatedly reacquire grants while other healthy workers could be skipped indefinitely.
- Added stale fair-waiter cleanup so a crashed or disappeared worker cannot permanently block a Keeper lane.
- Preserved fair queue position while a worker waits; after receiving one grant, the worker leaves the queue and rejoins at the back if it has additional work.
- Kept database locks scoped to coordination work only; workers do not hold PostgreSQL locks while asynchronously sleeping between grant attempts.
- Presence retains the durable last-known-good aggregate when current distributed coverage is unhealthy instead of replacing it with an incomplete player count.
- Draining workers remain excluded from Keeper ownership and Discord leadership while meaningful pre-drain cadence telemetry is preserved for eligible-worker adaptive calculations.

### Safety
- The cluster-wide Keeper request-start ceiling remains unchanged at `0.33` requests/second.
- The validated lane budgets remain `0.23` requests/second for bulk and `0.10` requests/second for fast/default polling; adding workers does not multiply those aggregate ceilings.
- Existing deterministic HRW Keeper ownership and fast/default lane partitioning remain unchanged.
- The legacy no-lane Keeper path retains its prior request-gate behavior.
- No Discord/database side-effect ownership was distributed; the fenced Discord leader remains the single processor for map changes, player history, watched-player alerts, persistent displays, and presence.

### Validation
- Stress-tested the distributed Keeper workload at 329 unique active BF4 servers across five workers.
- Confirmed the pre-fix starvation condition under sustained load, including healthy workers completing no bulk sweeps while another worker repeatedly completed its workload.
- Confirmed the fair gate restored progress across all five workers while preserving the existing global/lane request budgets.
- After fairness recovery, active snapshot coverage improved from 236/329 to 325/329, demonstrating that the previous missing-snapshot backlog was primarily scheduling starvation rather than insufficient aggregate rate.
- Confirmed the remaining long-stale tail was dominated by real server-specific external failures such as Keeper HTTP 404 responses rather than worker starvation.
- Validated adaptive presence at scale with healthy coverage above the configured 90% threshold once meaningful lane cadence telemetry was available.
- Validated durable presence hydration across Discord leadership handoffs.
- Validated PR2-C zero-work cadence preservation by confirming a subsequent zero-assignment sweep did not replace the preceding meaningful bulk cadence row.
- Completed the final five-node `v3.0.1` production rollout across `rnt-01`, `hnl-01`, `kah-01`, `mak-01`, and `mak-02`.
- During final deployment, validated a clean Discord leadership handoff from `mak-01` generation 48 to final-release `rnt-01` generation 49, including successful hydration of generation 48's persisted last-good presence aggregate.

## [v3.0.1-pr2c] - 2026-09-02

### Fixed
- Prevented an existing meaningful Keeper lane cadence observation from being overwritten by a later zero-assignment sweep.
- Preserved the last non-zero traversal-cadence measurement across worker drains, restarts, rolling deployments, and temporarily empty HRW assignments.
- Allowed telemetry max-age policy to expire preserved observations naturally rather than replacing them with synthetic idle cadence.

### Validation
- Verified on `mak-01` that a meaningful bulk sweep with 76 assigned servers persisted its cadence and completion data.
- Drained the worker and confirmed the immediately following zero-assignment bulk sweep left that meaningful telemetry row unchanged.
- Rolled PR2-C across all five workers and validated a generation 47 → 48 Discord leadership handoff with durable presence hydration and healthy adaptive snapshot evaluation.

## [v3.0.1-pr2b] - 2026-09-01

### Added
- Added `keeper_lane_worker_state` for durable per-worker/per-lane Keeper sweep telemetry.
- Added `presence_aggregate_state` for durable last-known-good distributed presence state.
- Added database-backed settings for snapshot cadence multipliers, adaptive horizon bounds, telemetry maximum age, and persisted-fallback validity.
- Added Alembic revision `0019_v3_0_1_presence_health`.

### Changed
- Distributed presence now derives separate bulk and fast snapshot horizons from observed eligible-worker Keeper cadence.
- When no usable cadence telemetry exists, presence falls back conservatively to the existing 900-second snapshot horizon.
- Fast/default servers use the fast horizon while the fast lane is active; bulk cadence remains the fallback when the fast lane is unavailable.
- Healthy presence aggregates are persisted with player/server counts, usable coverage, source worker, and Discord leadership generation.
- A newly elected Discord leader hydrates its process-local last-good player aggregate from durable state when the persisted aggregate is still within the adaptive fallback-validity window.

### Fixed
- Removed dependence on process-local presence fallback state during Discord leadership changes.
- Corrected the fixed 15-minute distributed-presence assumption that was incompatible with the real 24–34 minute Keeper bulk traversal observed at 329-server scale.

### Validation
- Demonstrated healthy adaptive presence at 329-server scale with 299 usable snapshots, 26 stale snapshots, 4 missing snapshots, and 90.88% usable coverage.
- Demonstrated generation 46 → 47 leader handoff hydration from the persisted last-good aggregate.
- Identified during validation that zero-work sweeps could overwrite meaningful cadence telemetry, leading directly to PR2-C.

## [v3.0.1-pr2] - 2026-08-31

### Added
- Added durable PostgreSQL fair-waiter queues for `keeper_bulk` and `keeper_fast`.
- Added `keeper_rate_waiters`, keyed by gate/lane and worker, with stale-waiter cleanup.
- Added aggregate per-sweep `gate_wait_seconds` telemetry.
- Added Alembic revision `0018_v3_0_1_keeper_fair`.

### Changed
- Keeper request coordination now separates fairness from pacing: the waiter queue determines which worker may proceed, while the existing global and lane rate-gate timestamps determine when the request may start.
- A waiting worker retains its original queue position until it receives a grant or becomes stale.
- After one grant, the worker leaves the queue and rejoins at the back for subsequent work.
- Bulk and fast lanes maintain independent fair queues while every request still passes through the existing hard global Keeper gate.
- Legacy no-lane Keeper requests continue to use the previous coordination path.

### Fixed
- Fixed severe distributed Keeper worker starvation caused by first-arriver-wins polling of the PostgreSQL request gate.
- Prevented healthy workers with HRW-owned servers from being skipped indefinitely while another healthy worker repeatedly reacquired grants.

### Safety
- Kept `keeper.external_requests_per_second=0.33`, `keeper.bulk_requests_per_second=0.23`, and `keeper.fast_requests_per_second=0.10` unchanged.
- Fairness does not create per-worker rate budgets and therefore does not multiply external request traffic as the worker count increases.
- Stale waiter leases are removed so a crashed worker cannot permanently hold the head of a lane queue.

### Validation
- Reproduced the pre-fix starvation at 329 unique servers: `hnl-01` and `kah-01` completed no bulk sweeps during a six-hour observation while other workers continued to complete work.
- After PR2, all five workers completed their HRW-owned bulk workloads with observed completion times of roughly 24–34 minutes under the unchanged shared rate limits.
- Verified the bulk ownership totals plus the 12 fast/default servers accounted for the complete 329-server active set.
- Verified active snapshot coverage increased to 325/329 after fair scheduling removed the starvation backlog.

## [v3.0.1-pr1] - 2026-08-30

### Added
- Added `PRESENCE_DISTRIBUTED_MIN_SUCCESS_RATIO=0.90` for distributed rich-presence health evaluation.
- Added adaptive persistent player-list refresh ETA estimation from observed process-local player-display cadence.

### Changed
- Distributed presence no longer requires every configured server to have a fresh snapshot; isolated missing/stale snapshots are tolerated when fresh coverage remains at or above 90%, no Keeper service failures occur, and the circuit breaker remains closed.
- Legacy/non-distributed presence health remains strict.
- Player-list ETA estimates use recent observed start-to-start cadence, with up to six samples, a fallback before observations are available, and a 60–900 second clamp.

### Validation
- At the original 69-server production scale, confirmed that 64/69 fresh snapshots (92.75%) remained healthy and allowed rich presence to update naturally.
- Observed adaptive player-list ETA stabilize around the actual ~139–143 second display cadence.
- Five-worker stress expansion to 329 unique servers exposed two additional scale issues: PostgreSQL Keeper request-gate starvation and the incompatibility of a fixed 15-minute snapshot horizon with the real distributed bulk traversal time. Those findings led to PR2 and PR2-B.

## [v3.0.0-pr4-d] - 2026-08-27

### Added
- Added the PR4-D fast/default Keeper scheduling lane for globally deduplicated default/high-priority server GUIDs.
- Added independent PostgreSQL-backed `keeper_bulk` and `keeper_fast` lane gates beneath the existing hard global Keeper request gate.
- Added `keeper.fast_enabled`, `keeper.fast_sweep_seconds`, `keeper.bulk_requests_per_second`, and `keeper.fast_requests_per_second` runtime settings.
- Added `keeper_fast` eligibility for the existing four-site v3 worker pool.
- Added `pr4d_static_checks.py` and `PR4_D_TESTING.md`.

### Changed
- When the fast lane is active and healthy, default GUIDs are removed from the bulk scope and assigned independently with the same deterministic HRW algorithm; the two lane scopes are disjoint.
- If no healthy `keeper_fast` worker is eligible, bulk retains default GUIDs as a fail-safe rather than allowing high-priority servers to become unpolled.
- `/operator workers drain` autocomplete now shows only non-draining workers; `/operator workers resume` shows only draining workers. Both handlers also reject stale/manual no-op state changes with friendly responses.

### Safety
- Migration 0016 seeds `keeper.fast_enabled=false`; the fast lane is not activated merely by rolling out PR4-D.
- Seeded lane rates are 0.23 req/s bulk + 0.10 req/s fast, while every request must also acquire the existing 0.33 req/s global Keeper gate. Additional workers/IPs therefore do not multiply the aggregate request-start ceiling.
- Fast-lane activation is a database/operator rollout step only after all workers are on PR4-D.

## [v3.0.0-pr4-c] - 2026-08-27

### Added
- Added operator-controlled worker draining with `/operator workers drain` and `/operator workers resume`, including worker autocomplete.
- Added explicit `Draining` visibility to `/operator status`; draining workers remain healthy/heartbeating while showing `Keeper: 0`.
- Added `pr4c_static_checks.py` and `PR4_C_TESTING.md` for rolling-upgrade validation.

### Changed
- Formalized the existing `cluster_workers.draining` field as the rolling-maintenance gate. Keeper HRW eligibility and Discord leadership eligibility already exclude draining workers; PR4-C exposes and validates that behavior end to end.
- Persisted drain state remains authoritative across worker/container restarts. Clearing draining is an explicit operator action.

### Safety
- PR4-C adds no database migration; Alembic remains at `0015_v3_0_0_keeper_dist`.
- Git/Docker deployment remains operator-controlled. Server Watcher does not remotely restart or rebuild worker hosts.

## [v3.0.0-pr4-b2-hf1] - 2026-08-27

### Fixed
- Reconciles the fenced Keeper processor when `keeper.distributed_enabled` hot-reloads, so an already-active non-rnt Discord leader starts processing without requiring a leadership handoff.
- Makes the monitor processor explicitly generation-owned and singleton-scoped; leadership loss cancels and awaits the generation's monitor before a replacement can be started locally.
- Adds cooperative yields between DB-heavy per-server processor work so Discord lease renewal remains schedulable during long snapshot/player-history cycles.
- Preserves the B2 kill switch behavior: with distributed mode disabled, only `rnt-01` runs the legacy Keeper processor.

### Validation
- Added `pr4b2hf1_static_checks.py` covering hot-enable reconciliation, generation ownership, singleton processor behavior, cooperative lease-renewal yields, the corrected `scope_name` migration schema, and existing HRW invariants.

All notable changes to BF4 Server Watcher are recorded here.


## [v3.0.0-pr4-b2] - 2026-08-27

### Added
- Added distributed Keeper snapshot acquisition: every eligible worker polls only its current HRW-owned live server GUIDs.
- Added PostgreSQL-backed `keeper_snapshots` as the shared handoff between distributed acquisition workers and the fenced Discord leader processor.
- Added DB-backed distributed sweep and snapshot freshness settings.

### Changed
- The global PostgreSQL 0.33 req/s request-start gate remains mandatory for every distributed Keeper request.
- Discord/database side effects remain single-writer: only the current fenced Discord leader processes shared snapshots into map changes, player history, watched-player alerts, persistent displays, and presence.
- HRW ownership is rechecked immediately before each external request so stale/draining worker changes remap safely.

### Safety
- `keeper.distributed_enabled` remains the live kill switch. With it false, legacy rnt-01 Keeper execution is preserved.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and the project uses semantic versioning.

## [v3.0.0-pr4-b1] - 2026-08-27

### Added
- Added a PostgreSQL-coordinated singleton Keeper request-start gate for a true cluster-wide request budget.
- Added durable gate observability (`last_worker_id`, `total_grants`, and `next_request_at`).

### Changed
- Keeper snapshot requests now acquire the database-backed global gate instead of the process-local external-request gate.
- Distributed Keeper execution remains disabled; PR4-B1 intentionally validates the new gate with the existing single Keeper executor first.

### Safety
- Missing or unavailable global rate-gate state fails closed for Keeper requests; workers do not silently fall back to independent per-host rates.

## [v3.0.0-pr4] - 2026-08-27

### Added
- Added PR4-A deterministic HRW/rendezvous Keeper assignment planning across healthy eligible workers.
- Added Keeper capability reporting and prospective per-worker Keeper ownership counts to `/operator status`.
- Added worker application versions to `/operator status`.
- Added `keeper.distributed_enabled` as a safety switch, seeded disabled for PR4-A.

### Changed
- The four existing v3 sites are seeded with the `keeper_bulk` role for assignment planning.
- Live Keeper polling intentionally remains on `rnt-01` while PR4-A validates ownership; aggregate Keeper traffic is unchanged.

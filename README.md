...
## v2.6.6 Pre-Release 2 — validated pacing and operations

PR2 promotes the controlled Keeper testing results and adds the operational changes queued before v2.7.0.

- Uses the validated pacing baseline: `EXTERNAL_REQUESTS_PER_SECOND=0.33`, `KEEPER_BATCH_SIZE=40`, `KEEPER_BATCH_PAUSE_SECONDS=120`, `KEEPER_INTER_SWEEP_COOLDOWN_SECONDS=120`.
- Keeps PR1's 403-flood protection and existing Keeper service-failure circuit breaker.
- Polls globally deduplicated default-server GUIDs first in every sweep, then the remaining tracked servers. A server shared by several guilds is still fetched only once.
- Extends `/delserver` with bulk choices for all non-default PC, PlayStation, or Xbox servers in the invoking guild. Default servers are never bulk-deleted.
- When an explicitly watched player is already online during the bot's initial player-history baseline, sends a one-time `currently online` informational alert instead of falsely saying the player just joined. Ordinary startup alerts remain suppressed.
- Recovery baselines after transient Keeper/network failures do not emit startup-online alerts.
...
v2.6.3 corrects the Keeper HTTP 403 classifier that remained breaker-eligible in v2.6.2.

- Keeper HTTP 403 now explicitly classifies as an isolated/transient failure and cannot increment the global Keeper circuit-breaker streak.
- The existing per-server 403 cooldown/backoff remains in place.
- Keeper HTTP 429 remains a global rate-limit signal.
- Keeper HTTP 5xx, timeout, and connection failures remain eligible for the global circuit breaker.
- The confirmed-working v2.6.2 `!list` 1850-character chunking and rich-presence safeguards are preserved.
- No database migration is required.
...
v2.6.0 replaces fully serial external server lookups with bounded concurrency and a shared global rate limiter. Keeper polling and Battlelog persona enrichment use a small worker pool (default 3 workers) while request starts remain globally paced at a conservative default of 1.0 request/second. This allows independent HTTP work to overlap without increasing aggregate request pressure beyond the configured limit.

The existing unique-server GUID deduplication, cached-result reuse, Keeper failure isolation, circuit breaker/backoff, Battlelog enrichment retry behavior, and watched-player identity promotion are preserved. `EXTERNAL_LOOKUP_WORKERS` and `EXTERNAL_REQUESTS_PER_SECOND` are configurable in `.env`. No database migration is required from v2.5.4.
